...
bool Transform2D::removeChild(int index)
{
    ...
    if (index > m_childCount - 1 || index < 0)
    {
        return false;
    }
    ...
    for (int i = 0; i < m_childCount; i++)
    {
        if (i == index)
        {
            //change IsRemoved to true
            //use continue keyword to skip saving this child and increasing the index of the new array.
            isRemoved = true;
            continue;
        }
        //store the new child in the empty spot at the end of the array
        temp[tempIndex] = m_children[i];
        //interate forward if the children added is increased
        tempIndex++;
    }

    if(isRemoved)
    {
        //cleanup the old array no longer being referenced
        delete[] m_children;
...
bool Transform2D::removeChild(Transform2D* child)
{
    ...
    for (int i = 0; i < m_childCount; i++)
    {
        ...
        {
            //change IsRemoved to true
            //use continue keyword to skip saving this child and increase the index of the new array.
            isRemoved = true;
            continue;
        }
        //store the new child in the empty spot at the end of the array
